...
```{r}
#Single Words
words1<-dfm(myCorpus, ngrams = 1,verbose = TRUE, toLower = TRUE,
  removeNumbers = TRUE, removePunct = TRUE, removeSeparators = TRUE,
  removeTwitter = TRUE, stem = FALSE,ignoredFeatures=stopwords("english")) 
words1<-topfeatures(words1, 30)
words1<-data.frame(cbind(names(words1),words1),stringsAsFactors =FALSE)
rownames(words1)<-NULL
colnames(words1)<-c("word","freq")
words1$freq<-as.numeric(words1$freq)

ggplot(words1, aes(reorder(word,freq), freq)) +
